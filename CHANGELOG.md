...
- `copy` command for transferring files between two configured sessions via SCP
  - `russh copy <source> <source-path> <dest> [dest-path]`
  - Destination path defaults to `~` when omitted
  - `--dry-run` / `-n` flag to preview without executing
  - **Jump-host aware**: when source and destination use different jump hosts,
    automatically falls back to a two-step copy via local temp file so each
    leg routes through the correct bastion
- `build_host_copy_args`, `build_download_args`, `build_upload_args`,
  `host_copy_strategy`, and `run_scp` in `russh-core::sync`
- `CopyStrategy` enum (`Direct` | `ViaLocal`) for host-to-host transfers
...
- README for `russh-core` crate with module overview and usage examples
- README for `russh-cli` crate with command reference and config examples
...
[1.1.0]: https://github.com/lacrioque/russh/compare/v1.0.3...v1.1.0
